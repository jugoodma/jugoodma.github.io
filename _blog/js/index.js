...
for (var i = list.length; i--; ) {
  // reverse
  list[i].onchange = (function (k) {
    return function () {
      sortTags(k);
    };
  })(i); // gotta use IIFE to create closure to save value of 'i'
}
...
function updatePosts(tag, hide) {
  ...
  for (var i = list.length; i--; ) {
    ...
    if (list[i].getAttribute("data-tags").split("|").includes(tag)) {
      if (hide) list[i].style.visibility = "hidden";
...
for (var i = list.length; i--; ) {
  list[i].onclick = function (e) {
    ...
    if (hide) e.target.classList.remove("active");
